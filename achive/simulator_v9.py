# ==============================================================================
# Valorium X: Python Simulator v9.1 - The Final Audit (Compactor Escape)
# Architect: Sylver Blade, Assisted by Gemini
#
# This version includes critical import fixes to ensure all modules,
# especially the logging framework, function correctly. This is the definitive,
# stable, single-file version for testing.
# ==============================================================================
...
class Block:
    ...
    def from_dict(cls, data: dict):
        """Creates a Block instance from a dictionary, robustly handling potential missing keys."""
        ...
        winning_cip_data = data.get('winning_cip_proof')
        if winning_cip_data:
            block.winning_cip_proof = CipProof(**winning_cip_data)
        
        if data.get('attestations') and block.winning_cip_proof:
            for att_data in data.get('attestations', []):
                temp_cip_proof = CipProof(block.rna_template_hash, "") 
                temp_cip_proof.proof_hash = att_data.get('proof_hash')
                attestation_obj = CipAttestation(temp_cip_proof, att_data.get('node_address'))
                attestation_obj.signature = att_data.get('signature')
                block.attestations.append(attestation_obj)
    ...
    def calculate_hash(self) -> str:
        ...
        block_data.pop('block_hash', None)
...
class Blockchain:
    ...
    def add_transaction(self, tx: Transaction):
        if tx.sender != "Network Reward" and self.balances.get(tx.sender, 0) < tx.amount:
            logging.warning(f"Transaction from {tx.sender} for {tx.amount} VQXAI rejected: Insufficient funds.")
            return False
    ...
    def process_block_creation_cycle(self):
        ...
        if len(participating_nodes) < (len(self.neural_nodes) // 2 + 1): # Need a majority of total nodes to even try
            logging.error(f"FAILURE: Not enough reputable and compliant Neural Nodes online for consensus.")
        ...
        logging.info(f"Consensus: Checking for consensus on proof {core_cip_proof.proof_hash[:8]}... (Threshold: {cip_consensus_threshold} attestations)")
        
        proof_counts = {}; [proof_counts.update({att.proof_hash: proof_counts.get(att.proof_hash, 0) + 1}) for att in attestations]
        
        winning_proof_hash, winning_count = max(proof_counts.items(), key=lambda item: item[1]) if proof_counts else (None, 0)

        for node in participating_nodes:
            attestation = next((att for att in attestations if att.node_address == node.address), None)
            if not attestation or attestation.proof_hash != winning_proof_hash:
    ...
    def load_from_file(cls, filename: str, validator_nodes, neural_nodes, stencil):
        """Loads a blockchain state from a JSON file."""
        try:
            with open(filename, 'r') as f:
                state = json.load(f)
            logging.info(f"Loading blockchain state from {filename}...")
            loaded_blockchain = cls(validator_nodes, neural_nodes, stencil)
            loaded_blockchain.chain = [Block.from_dict(block_data) for block_data in state.get('chain', [])]
            loaded_blockchain.balances = state.get('balances', {})
            loaded_blockchain.pending_transactions = [Transaction(**tx_data) for tx_data in state.get('pending_transactions', [])]
            # In a real system, node state (stake, reputation) would also be persisted and restored.
            return loaded_blockchain
        except FileNotFoundError:
            logging.info(f"State file {filename} not found. Starting with a new blockchain.")
            return None
...
if __name__ == '__main__':
    logging.info("--- VALORIUM X SIMULATOR V9: DEFINITIVE EDITION ---")
    ...
    if valorium_chain is None:
        valorium_chain = Blockchain(validator_nodes, neural_nodes, stencil)
        logging.info("New blockchain instance created. Initializing stakes and balances...")
        valorium_chain.initialize_chain()
    ...
    for node in (validator_nodes + neural_nodes):
        logging.info(f"  - {node.address}: {node.reputation:.2f}")